...
mysql> system sudo  ls -lh /usr/local/mysql/data/login | grep t
total 2288
...
-rw-r-----  1 _mysql  _mysql    96K  8 30 16:06 t#P#p0.ibd
-rw-r-----  1 _mysql  _mysql    96K  8 30 16:06 t#P#p1.ibd
-rw-r-----  1 _mysql  _mysql   8.4K  8 30 16:06 t.frm
-rw-r-----  1 _mysql  _mysql    40B  8 23 14:49 tgr_usercash_update.TRN
...
mysql> insert into t (id) values (15);
Query OK, 1 row affected (0.00 sec)

mysql> insert into t (id) values (30);
      ...
      SUBPARTITION_EXPRESSION: NULL
        PARTITION_DESCRIPTION: 10
              ...
              MAX_DATA_LENGTH: NULL
                 INDEX_LENGTH: 0
                    DATA_FREE: 0
                  CREATE_TIME: 2019-08-30 16:06:18
                  UPDATE_TIME: 2019-08-30 16:28:12
                   CHECK_TIME: NULL
                     CHECKSUM: NULL
            PARTITION_COMMENT: 
                    NODEGROUP: default
        ...
        PARTITION_DESCRIPTION: 20
              ...
              MAX_DATA_LENGTH: NULL
                 INDEX_LENGTH: 0
                    DATA_FREE: 0
                  CREATE_TIME: 2019-08-30 16:06:18
                  UPDATE_TIME: 2019-08-30 16:28:31
                   CHECK_TIME: NULL
                     CHECKSUM: NULL
            PARTITION_COMMENT: 
                    NODEGROUP: default
...
查询information\_schema架构下的PARTITION表来查看每个分区的具体信息，TABLE\_ROWS为每个分区中记录的数量，PARTITION\_METHOD为分区的类型。插入一个不在分区中定义的值时，会抛出一个异常。


```
mysql> CREATE TABLE sales ( money INT UNSIGNED NOT NULL, date DATETIME ) ENGINE=INNODB PARTITION by RANGE (YEAR(date)) ( PARTITION p2018 VALUES LESS THAN (2019), PARTITION p2019 VALUES LESS THAN (2020) );
Query OK, 0 rows affected (0.03 sec)

mysql> insert into sales (money,date) values(100,'2018-01-01');
Query OK, 1 row affected (0.00 sec)

mysql> insert into sales (money,date) values(100,'2018-02-01');
Query OK, 1 row affected (0.00 sec)

mysql> insert into sales (money,date) values(100,'2018-01-02');
Query OK, 1 row affected (0.00 sec)

mysql> insert into sales (money,date) values(100,'2019-03-02');
Query OK, 1 row affected (0.00 sec)

mysql> insert into sales (money,date) values(100,'2020-03-02');
ERROR 1526 (HY000): Table has no partition for value 2020
mysql> select * from sales;
+-------+---------------------+
| money | date                |
+-------+---------------------+
|   100 | 2018-01-01 00:00:00 |
|   100 | 2018-02-01 00:00:00 |
|   100 | 2018-01-02 00:00:00 |
|   100 | 2019-03-02 00:00:00 |
+-------+---------------------+
4 rows in set (0.00 sec)

mysql> explain partitions select * from sales where date>='2018-01-01' and date<='2018-12-31'\G
*************************** 1. row ***************************
           id: 1
  select_type: SIMPLE
        table: sales
...
mysql> explain partitions select * from sales where date>='2018-01-01' and date<='2019-12-31'\G
*************************** 1. row ***************************
           id: 1
  select_type: SIMPLE
        table: sales
   partitions: p2018,p2019
         type: ALL
possible_keys: NULL
          key: NULL
      key_len: NULL
...
通过EXPLAIN PARTITIONS可查看查询条件搜索的分区，SQL优化器会做分区修剪。可根据分区直接删除指定范围内的数据。
...
mysql> alter table sales drop partition p2018;
...
对于RANGE分区的查询，优化器只能对YESR()、TO_DAYS()、TO_SECONDS()、UNIX_TIMESTAMP()这类函数进行优化选择。

##### LIST分区
LIST分区相似于RANGE分区，分区列的值是离散的，而非连续。

```
mysql> create table t ( a int, b int)ENGINE=INNODB PARTITION BY LIST(b) ( PARTITION p0 VALUES in (1,3,5,7,9), PARTITION p1 VALUES in (0,1,2,4,6,8));
...
mysql> insert into t (a,b) values (1,1);
Query OK, 1 row affected (0.01 sec)

mysql> insert into t (a,b) values (1,2);
Query OK, 1 row affected (0.00 sec)

mysql> insert into t (a,b) values (1,6);
Query OK, 1 row affected (0.00 sec)

mysql> insert into t (a,b) values (1,7);
Query OK, 1 row affected (0.01 sec)

mysql> insert into t (a,b) values (1,10);
ERROR 1526 (HY000): Table has no partition for value 10
mysql> select table_name,partition_name,table_rows from information_schema.PARTITIONS where table_name='t' and table_schema=DATABASE()\G;
*************************** 1. row ***************************
    table_name: t
partition_name: p0
    table_rows: 2
...
VALUES IN中的值又重复时创建表会报错，插入值不在其中会报错。

INSERT插入多个数据的过程中遇到分区未定义时，MyISAM和InnoDB存储引擎表现不同，MyISAM会将之前的行数据都插入，之后的数据不插入。InnoDB将其视为一个事务，没有任何数据插入。


##### HASH分区

目的将数据均匀地分布到预先定义的各个分区中，保证各分区的数据数量大致都是一样的。需要基于将要进行哈希分区的列值指定一个列值或表达式，以及指定被分区的表将要被分割成的分区数量。
...
mysql> select table_name,partition_name,table_rows from information_schema.PARTITIONS where table_name='t_hash' and table_schema=DATABASE()\G;
*************************** 1. row ***************************
    table_name: t_hash
partition_name: p0
    table_rows: 0
...
对于连续的值进行HASH分区，可以较好的将数据进行平均分区。

LINEAR HASH分区，使用更加复杂的算法确定新行插入到已经分区的表中的位置。

```
mysql> create table t_linear_hash ( a int, b datetime )engine=innodb partition by linear hash (year(b)) partitions 4;
Query OK, 0 rows affected (0.03 sec)

mysql> insert into t_linear_hash values (1,'2010-04-01');
Query OK, 1 row affected (0.01 sec)

mysql> select table_name,partition_name,table_rows from information_schema.PARTITIONS where table_name='t_linear_hash' and table_schema=DATABASE()\G;
*************************** 1. row ***************************
 ...
 * 所在分区N=YEAR('010-04-01')&(V-1)=2

LINEAR HASH分区的优势在于增加、删除、合并和拆分分区变得更加快捷，有利于处理含有大量数据的表。缺点在于各分区数据的分布可能不大均衡。


##### KEY分区

与HASH分区相似，HASH分区使用用户定义的函数进行分区，KEY分区使用MySQL数据库提供的函数进行分区。对于NDB Cluster引擎，MySQL DB使用MD5函数来分区；其他存储引擎使用内部分哈希函数，这些函数基于PASSWORD()一样的运算法则。
...
Query OK, 0 rows affected (0.05 sec)
```
在key分区中使用关键字LINEAR和在HASH分区中使用具有相同的效果，分区编号通过2的幂算法得到而不是通过模数算法。

##### COLUMNS分区

RANGE、LIST、HASH、和KEY分区条件：数据必须是整型，或通过函数将其转化为整型。

MySQL v5.5开始支持COLUMNS分区，是RANGE分区和LIST分区的进化，可以直接使用非整型数据进行分区，分区根据类型直接比较而得，不需要转化为整型。RANGE COLUMNS分区可对多个列的值进行分区。

COLUMNS分区支持的数据类型：

 * 所有整型类型，如INT、SMALLINT、TINYINT、BIGINT。FLOAT和DECIMAL不支持。
 * 日期类型，DATE和DATETIME。其余日期类型则不支持。
...
mysql> select table_name,partition_name,table_rows from information_schema.PARTITIONS where table_name='t_columns_range' and table_schema=DATABASE()\G
*************************** 1. row ***************************
    table_name: t_columns_range
partition_name: p0
    table_rows: 0
*************************** 2. row ***************************
    table_name: t_columns_range
...
mysql> select table_name,partition_name,table_rows from information_schema.PARTITIONS where table_name='rcx' and table_schema=DATABASE()\G
*************************** 1. row ***************************
    table_name: rcx
partition_name: p0
    table_rows: 0
...
在分区的基础上再进行分区，也称符合分区。MySQL DB允许再RANGE和LIST的分区上再进行HASH或KEY的子分区。
...
mysql> system sudo ls -lh /usr/local/mysql/data/test | grep ts
Password:
-rw-r-----  1 _mysql  _mysql    96K  8 30 19:27 ts#P#p0#SP#p0sp0.ibd
-rw-r-----  1 _mysql  _mysql    96K  8 30 19:27 ts#P#p0#SP#p0sp1.ibd
-rw-r-----  1 _mysql  _mysql    96K  8 30 19:27 ts#P#p1#SP#p1sp0.ibd
-rw-r-----  1 _mysql  _mysql    96K  8 30 19:27 ts#P#p1#SP#p1sp1.ibd
-rw-r-----  1 _mysql  _mysql    96K  8 30 19:27 ts#P#p2#SP#p2sp0.ibd
-rw-r-----  1 _mysql  _mysql    96K  8 30 19:27 ts#P#p2#SP#p2sp1.ibd
-rw-r-----  1 _mysql  _mysql   8.4K  8 30 19:27 ts.frm

mysql> create table ts_subpartition (a int,b date)engine=innodb                                                                                                                                                                                                                     
    -> partition by range (year(b))
    -> subpartition by hash(to_days(b)) (
    -> partition p0 values less than (2019) (subpartition s0, subpartition s1)
    -> ,partition p1 values less than (2020) (subpartition s2, subpartition s3)
    -> ,partition p2 values less than maxvalue (subpartition s4, subpartition s5));
Query OK, 0 rows affected (0.05 sec)

mysql> system sudo ls -lh /usr/local/mysql/data/test | grep ts_subpartition
Password:
-rw-r-----  1 _mysql  _mysql    96K  8 30 19:37 ts_subpartition#P#p0#SP#s0.ibd
-rw-r-----  1 _mysql  _mysql    96K  8 30 19:37 ts_subpartition#P#p0#SP#s1.ibd
-rw-r-----  1 _mysql  _mysql    96K  8 30 19:37 ts_subpartition#P#p1#SP#s2.ibd
-rw-r-----  1 _mysql  _mysql    96K  8 30 19:37 ts_subpartition#P#p1#SP#s3.ibd
-rw-r-----  1 _mysql  _mysql    96K  8 30 19:37 ts_subpartition#P#p2#SP#s4.ibd
-rw-r-----  1 _mysql  _mysql    96K  8 30 19:37 ts_subpartition#P#p2#SP#s5.ibd
-rw-r-----  1 _mysql  _mysql   8.4K  8 30 19:37 ts_subpartition.frm
```
...
子分区可用于特别大的表，在多个磁盘分别分配数据和索引。


#### 分区中的NULL值

MySQL允许对NULL值做分区，总是视NULL值小于任何一个非NULL的值，与MySQL DB中处理NULL值的ORDER BY操作是一样的。对于不同的分区类型，MySQL DB对NULL值的处理也各不相同。

对RANGE分区，若向分区列插入NULL值，则将该值放入最左边的分区。

在LIST分区下使用NULL值需要显式地指定哪个分区中放入NULL值，否则报错。

HASH和KEY分区处理NULL值时，任何分区函数都会将含有NULL值的记录返回为0。

#### 分区和性能

数据库应用分为：OLTP（在线事务处理），如Blog、电子商务、网络游戏等；OLAP（在线分析处理），如数据仓库、数据集市。

对OLAP应用，分区一般能很好地提高查询的性能。OLAP应用大多数查询需要频繁地扫描一张很大的表。

对OLTP应用，分区需要注意，用用通常不可能会获取一张大表中10%的数据，大部分都可通过索引返回几条记录即可。对一张大表，一般的B+ Tree需要2～3次的磁盘IO。设计时需要确认数据的访问模式，分析查询时使用分区是否能够比未使用分区减少IO访问次数。


#### 在表和分区间交换数据

MySQL v5.6开始支持ALTER TABLE ... EXCHANGE PARTITION语法。允许分区或子分区中的数据与另一个非分区的表中的数据进行交换。若非分区表中的数据为空，则相当于将分区中的数据迁移到非分区表中。若分区表中的数据为空，则相当于将外部表中的数据导入到分区中。
